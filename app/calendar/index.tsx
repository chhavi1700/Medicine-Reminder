...
export default function CalendarScreen() {
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= days; day++) {
      ...
      const hasEvents =
        getScheduleEntriesForDate(medications, doseHistory, date).length > 0;
  ...
  const renderMedicationsForDate = () => {
    const entries = getScheduleEntriesForDate(
      medications,
      doseHistory,
      selectedDate
    );
    ...
    return entries.map((entry) => (
      <View key={entry.id} style={styles.medicationCard}>
        <View
          style={[
        ...
        <View style={styles.medicationInfo}>
          <Text style={styles.medicationName}>{entry.medication.name}</Text>
          <Text style={styles.medicationDosage}>{entry.medication.dosage}</Text>
          <Text style={styles.medicationTime}>{entry.time}</Text>
        </View>
        {entry.taken ? (
