...
export default function HomeScreen() {
  ...
  const [medicinesDueInNextHour, setMedicinesDueInNextHour] = useState<Medication[]>([]);
  ...
  const loadMedications = useCallback(async () => {
    try {
      ...
      // Get medicines due in next hour for notification badge
      const medicinesDue = getMedicinesDueInNextHour(allMedications, todaysDoses);
      setMedicinesDueInNextHour(medicinesDue);
  ...
  const handleTakeDose = async (medication: Medication) => {
    try {
      ...
      const updatedMedication = allMedications.find(
  ...
  const totalScheduledDoses = todaysMedications.reduce(
    (sum, med) => sum + med.times.length,
    0
  );
  const progress = totalScheduledDoses > 0 ? completedDoses / totalScheduledDoses : 0;
        ...
        <View style={styles.headerContent}>
          <View style={styles.headerTop}>
            ...
            >
              ...
              {medicinesDueInNextHour.length > 0 && (
                <View style={styles.notificationBadge}>
                  <Text style={styles.notificationCount}>
                    {medicinesDueInNextHour.length}
                  </Text>
                </View>
              )}
            </TouchableOpacity>
        ...
        <View style={styles.section}>
          ...
          {todaysMedications.length === 0 ? (
            ...
            todaysMedications.map((medication) => {
              const takenCount = doseHistory.filter(
                (dose) => dose.medicationId === medication.id && dose.taken
              ).length;
              const allTaken =
                medication.times.length > 0
                  ? takenCount >= medication.times.length
                  : takenCount > 0;
              return (
                <View
                  key={medication.id}
          ...
          >
            ...
            {medicinesDueInNextHour.length === 0 ? (
              <View style={styles.emptyNotificationState}>
                ...
                >
                  No medications due in the next hour
                </Text>
              </View>
            ) : (
              medicinesDueInNextHour.map((medication) => (
                <View
                  key={medication.id}
                  style={[
